...
Functional and nonfunctional requirements’ of iOS and server applications are based on the main functionalities of applications that i analyzed in previous section. Some of the requirements are also based on simplicity of use, eliminating problems of corporation-aimed tools and providing users native OS experience.

\subsubsection{Functional requirements}

\subsubsubsection{Authentication}
User can register by providing name, surname, birthday and email or log in to the application through email and password. The application supports two roles - student and teacher. Role is selected during registration, only users older than 18 years can be teachers. User can log out of the application and delete account if needed.

\subsubsubsection{List of tasks}
User with role teacher can see all tasks he created. User with role student can see all the tasks he was assigned, their current status, author and deadline. 

\subsubsubsection{Work with tasks}
User with role teacher can create tasks, assign them to his students and control task elaboration. User can provide comments and files for task elaboration in form of comments.

\subsubsubsection{List of students}
User with role teacher can add students and delete them. He laso can history of their tasks.

\subsubsubsection{User profile}
Every user has it's own profile and can upload profile picture. User with role student also sees his histor of ongoing and closed tasks.

\subsubsubsection{Feedback}
Every user can send feedback for app developer providing text and optionally screenshot of a problem.

\subsubsection{Nonfunctional reqirements}
\subsubsubsection{Operating system}
Mobile application supports operating system iOS 17.

\subsubsubsection{Frameworks}
Mobile application  uses SwiftUI as UI framework. Server application is written in Swift 5 using Vapor framework.

\subsubsubsection{Server architecture}
Server application implements REST API and conforms to its design principles.

\subsubsubsection{Authorization}
User is authorized through Bearer token for all endpoints that require authorization.

\subsubsubsection{User interface}
User interface is designed to be native and user-friendly. It utilizes best practices of native iOS development and is inspired by system iOS applications.
...
In this chapter i dive in software and graphic design aspects of the applications. In the first part I provide overview of design and architecture of mobile application and reasons for choosing it.
\subsection{Architecture}
I have chosen Clean Architecture, so whole application is separated into three layers according to the Clean Architecture principles. Layers are Presentation, Domain and Data Figure \ref{fig:image4} shows dependencies between layers. Further in the text I will dive into details of each layer.
...
Mobile app utilizes advanced modularisation with SPM(Swift Package Manager), so each layer is divided to logical parts. It also improves application build time, which is crucial during app development.

\subsubsection{Application Layer}
Application Layer isn't a part of traditional Clean Architecture representation, but is crucial for application lifecycle. As displayed in Figure \ref{fig:image5} it contains:
\begin{itemize}
	\item Dependency Injection Package - helps to utilize dependency inversion and single responsibility principles by decoupling the usage of objects from its creations. Use case, providers and toolkits are registered here, and then used in other parts of application.
	\item Info folder - contains all \texttt{Info.plist} and other privacy files needed for application start.
	\item AppDelegate - object that manages app's shared behaviors.
...
Presentation layer handles the user interface of application. It is represented by FlowControllers and ViewModels + Views. This layer follows MVI architecture - each ViewModel has its state and intents, which are then used in a relevant SwiftUI View. Asynchronous functions are represented via native \texttt{async/await}.
...
In this example, we then implement how the \texttt{ProfileFlowController} handles specific flows.
...
Each top level View has its own ViewModel, that hadle state of entire view and passes data via \texttt{State} struct and \texttt{onIntent(Intent)} function handles interaction with View.
...
Domain layer reflect the whole business logic of the application via DomainModels and UseCases. It also defines protocols for repositories. This layer is separated into logical parts by feature.
...
App should only use these domain models and not models from some third-party libraries, as that would make the app strictly dependent on those specific libraries.  These models are defined under the Models directory for each logical part.
...
Domain layer defines protocols for repositories - functions and their interfaces. Repositories are created for each logical part of applcaition.

\subsubsection{Data Layer}
Data layer is responsible for providing data via repositories and providers from network. It is separated into \texttt{Toolkits} and \texttt{Providers}. There is a toolkit for each logical part of the domain layer. These toolkits provide repository implementations and relevant \texttt{NetworkModels}, \texttt{NetworkEndpoints}, etc. \texttt{Providers} define provider protocols and implementations.
...
\subsection{Design}
In this section, I list designed key components and their responsibilities.

\subsubsection{Screens}
Here I present main Views of the iOS application. Each of them has corresponding viewmodel.
\begin{itemize}
...
In this chapter I dive into designing architecture of web application and explain my decisions.
...
Mobile application is created based on design provided in previous parts using primarily SwiftUI framework. \texttt{Tasks screen} that is considered as main application's screen is presented by \texttt{TasksView}. It consists of \texttt{TaskList} if tasks are present, and \texttt{ContentUnavailabelView} otherwise. The screen has title "Tasks" and button to create a new task if user role is teacher.
...
ViewModels have a specific structure to conform to an MVI architecture, so it always has some \texttt{State} and enum \texttt{Intent}, taht contains all the intents that this ViewModel must handle.
...
For communication with server \texttt{NetworkProvider Package} was created. It defines encodings used, provides protocol for describing \texttt{NetworkEndpoint} and \texttt{NetworkProvider}, implments \texttt{SystemNetworkProvider}.
...
Then data provided by repositories is passed to ViewModels via injected UseCases.
...
In order to authorize the client based on the token in the HTTP header, it was necessary to mark the given path and at the same time call the authorization method in the body of the function as shown below. Itis a simplified version of the UserController structure that registers a path to retrieve information about the user.
...
The code below shows a user registering. User inforamtion is obtained from HTTP body of a request. Then app checks whether user's email already exists in database. Aftert check, user is saved to db, accessToken is generated and sent back.
...
Some endpoints can be accessed only by users with the teacher role. To ensure this I have created \texttt{EnsureUserIsTeacherMiddleware}. This helps to verify user role, before reaching an endpoint and create configurable routing depending on logic of application. Middleware responds to request and either aborts it if requirements are not met, or continues routing it for an actual endpoint.
...
Deploying API and iOS app is a crucial step in bringing software to production. Below, I cover the deployment process for both components.

\subsection{Deploying the Server to Heroku}

Heroku is a popular platform-as-a-service (PaaS) that simplifies the deployment process. To deploy API server, I have:
...
\begin{enumerate}
    ...
    \item \textbf{Created a New Heroku App:} Created a new Heroku app using the \texttt{heroku create} command. This generates a unique URL for application.
    \item \textbf{Deployed Code:} Pushed code to Heroku's Git repository using the \texttt{git push heroku master} command. Heroku automatically builds and deploys application.
...
TestFlight is Apple's platform for distributing beta versions of iOS apps to testers before releasing them to the App Store. To publish iOS app to TestFlight, I have made these steps:

\begin{enumerate}
    \item \textbf{Prepared  app:} iOS app is fully tested and ready for beta testing. I've ensured it meets Apple's App Store Review Guidelines.
    \item \textbf{Created an App Store Connect Record:} Logged in to App Store Connect with Apple Developer account and created a new app record for my iOS app.
    \item \textbf{Build and Archive App:} Build my iOS app in Xcode and creatde an archive using the Archive feature.    
    \item \textbf{Uploaded Build to App Store Connect:} Used Xcode's Organizer window to upload the app archive to App Store Connect. This submits app for review and processing.
...
After doing these iOS app StudentChrono is available for public testing. Public test URL is \url{https://testflight.apple.com/join/ENm8Gj5q}
